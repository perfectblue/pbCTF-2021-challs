...
function AuthRoute({children, location, ...rest}) {
...
function ChatApp() {
  ...
  useEffect(() => {
    ...
    if (user?.uid) {
      unsubFromUserConnection = dispatch(checkUserConnection(user.room, user.uid));
    }
  ...
  if (!isOnline) {
    return <LoadingView message="Internet is disabled, please turn it back on." />
  }
